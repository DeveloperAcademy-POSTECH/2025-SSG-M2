...
==이 차이를 알아야 하는 이유는??
-> 개발자가 가장 중요하게 고려해야 할 것 중 하나는 성능이며 Class, Struct의 차이점을 확실히 알고 있다면 성능개선을 할 수 있다
...
|**복사 방식**|데이터를 **복사해서 전달**|데이터를 **참조해서 전달**|
...
class Dog {
    ...
    init(name: String) {
        ...
        // 속성쪽 name 이라고 알려 주는 것 , 지금 코드에선 상관 없지만 코드가 복잡해지면
        // 써줘야 함
    }
}
...
![[스크린샷 2025-05-25 오후 10.23.27.png]]
